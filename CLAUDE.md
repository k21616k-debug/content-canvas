...
### 最近完成的功能（commit 29d1a06）

1. **知識空洞指示器** — 節點卡 `📦?` 徽章（`!node.user && !node.aiResearch` 時顯示）
2. **AI Review 新增 merge/remove** — prompt + UI 按鈕都已落地
3. **Hook 選用按鈕** — 選定後設 `node.aiResearch.suggestedHook`
4. **CTA 套用按鈕** — 直接設 `node.main.cta`（不需要「採納全部」）
5. **YouTube 標題「用這個」按鈕** — 替換節點標題
6. **製作準備 bar** — 節點面板顯示 Hook/CTA/拍攝清單完成狀態
7. **filmingAngles `confirmed` 狀態** — AI expand 後預設 `false`，使用者勾選確認
8. **Brief 重構** — 執行清單排第一（Hook + 鏡頭清單 checkboxes + CTA），策略背景下移至折疊區
