...
| `suggest-node` | AI 深度推導出的漏斗缺口建議主題 | 點擊「AI 建議主題」或全局檢討發現缺口 |

`new-node` 舊模板類型已永久移除，不可再加。請使用 `suggest-node` 來處理深度推導的新影片主題。
...
**規則：** 每次完成 Content Canvas 的程式碼修改（不論大小），在報告「完成」之前，必須用 Workflow 跑一輪 octopus 多角色驗收。不用 Andrew 要求，自動跑。

**驗收內容（最低要求）：**
1. **API 測試** — 用 curl 打所有被改過的 endpoint，確認回傳格式正確
2. **Code Audit** — 多角色檢查：前後端欄位名對齊、JSON 解析、error handling、state/save 流程
3. **Adversarial Verify** — 對 critical findings 做反向驗證，確認是真 bug 不是誤報

**為什麼有這條：** v3 實作時，workflow 寫出的程式碼看起來完全正確，但 octopus 驗收抓到 4 個 critical bug（crossBrief 存 object 而非 string、delta 前端沒送 previousResearch、renderPanel 讀錯欄位、定價常數沒改）。每一個都是「跑得起來但行為是錯的」，人工看不出來。

**流程：** 實作 → octopus 驗收 → 修 findings → 報告完成。如果驗收有 critical，修完後要再跑一輪確認。
...
- 路徑：`/Users/chunhuiliu/Andrewspot-AI-Core/專案/content-canvas/`
- 啟動：`node --watch --env-file=content-canvas/.env content-canvas/server.js`（port 3456，--watch 自動重載 api/*.js 變動）
...
### 2026-05-30 主架構審查 + 必改 + 優化（已本機 commit，尚未 push）

**commit**：`64262b6`(發散+ideation)、`7b2dbb8`(架構必改)、`538b7c6`(優化收尾)。三批都本機 commit 在 main、**未 push**。

**做法**：架構審查 workflow(6 角色)→ 必改 → 整站驗證 workflow(6 角色)→ 4 收尾。**核心裁決:架構夠用、別大重構**(serializeNode/完整 node.sources/外部 API 全被反向架構師打回:premature/爛尾)。

**7 必改(commit 7b2dbb8)**：
- **#1/#2 多專案分檔存(最重要,資料安全)**：原本 `/api/save` 整檔覆寫 `canvas-data.json` → 只存得住最後一個專案,多專案靠 localStorage,清快取就掉。改 `data/{projectId}.json` 分檔(原子 tmp→rename + projectId 白名單擋 traversal);`saveToFile` 送 projectId + 800ms debounce;`data/` 進 git 當版本化備份。**已還原背包(8)+ funnel(7)**。⚠️ 背包之前真的從 localStorage 掉了,靠 `canvas-data.sim-backup.json` 救回。
- **#4** 砍 5 個封存死的 `interview-*` 端點 + interview.html/app(解 Vercel Hobby 12-func 上限 12→7)。
- **#5** server.js 補 `/api/script` 路由(本機原本拿假 mock)。**#6** 移除死的 `/api/parse` 鏈 174 行。**#7** `_usage` 補桶 + script/classify 接 `addUsage`。

**4 優化收尾(commit 538b7c6)**：① switchProject 加 `clearTimeout(_saveTimer)`(修本 session #2 debounce 開的跨專案污染)② 面板招牌「AI 解讀結果」→「發散候選」③ script/classify 接 addUsage ④ script.js `user?.notes`→`user` + `research.positioning`→`research.insight`。

**儲存模型(未來必讀)**：localStorage = per-project 主 store;server `data/*.json` = durable 備份(進 git);`canvas-data.json` 已 gitignore + 不再寫入(legacy)。**Vercel 存不住檔是 serverless 本質,付費也一樣 → 本機為主**;要部署得換 DB(爛尾風險,先不做)。

**可做(Andrew 取捨,別硬擠)**：loadState 冷啟 fallback 仍讀 legacy canvas-data.json(可拔或接 per-project);兩套 load 合一(消 topicListOrder 漂移);node.sources 查一次共用(把慢變快的真解,大架構,等產完第一支片);YouTube 競品/關鍵字接線(第2題下個洞,等上架後);deleteProject 不刪 server 檔(孤兒,git 裡無害)。

**誠實結論**：工具已過堪用線,瓶頸從「工具不夠好」移到「片還沒拍」。最高槓桿是**拿去拍第一支背包片**,別再加功能。

### 2026-05-31 後段小修（已 commit）+ 下次待辦

**又補 5 個 commit（本機 main、未 push）**：`e19b3ff`(快照)、`e0112c0`(hub 文案 解讀策略→研究擴寫)、`66d8c17`(健康列被浮動 #topic-toolbar 遮擋→.canvas-health-bar 加 padding-top:48px)、`68e90e4`(「快速上手流程」教學更新對齊真實流程:補 擴寫→採納→發散 + D 安心手動)、`746afe4`(divergeFromNode 加 `_divergeBusy` 旗標,防切視圖後再按重複付費呼叫)。

**⏳ 待 Andrew 決定（這 session 提了、還沒做）**：
1. **「✂ 切短片」新功能（Andrew 想要、最可能先做）**：聚焦一支長片 → AI 生 N 個短片剪輯點子（切哪段／Hook／平台）→ 採用變 material='short' 連回長片的節點。純推導（用節點已有 topic/角度/筆記，不上網查、不爛尾），跟發散同一套候選 UI 流程。**這是「分發優先」的真功能、Andrew 已明說需要，等他點頭就做、做完 live 測再 commit。**
2. 發散「載入中」面板切視圖會被 render() 藏（只有結果回來才由 renderDivergeCandidates 跳回）→ 可加「切回來時若 `_divergeBusy` 為真就重顯載入中」。已加防重複的 toast，夠用；這是加分項。
3. `applyResearch`(app-v2.js ~627) 的 CTA 是「只填空的」(保護手改)。Andrew 若想「CTA 永遠同步最新 AI 版」→ 拿掉該行 `&& !node.main.cta`（碼裡有 ⚑ Andrew 標記）。
4. 3 檔未 commit（`.claude/settings.json`、`GENERATION-LOOP-SPEC.md`、`mockup-diverge.html`）= config/doc，Andrew 決定 commit/gitignore。

**重申儲存與啟動**：localStorage 是 per-project 主 store；`data/*.json`(進 git)是 durable 備份；背包=`data/p1780050433417.json`(8 節點)。開工具走桌面捷徑「啟動內容企劃.command」（會自動開 product-v2.html）。

### 2026-05-30 ideation 強化（多角色 workflow 迴圈，已 commit 64262b6）
