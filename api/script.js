...
影片目的：${job || '未指定'}
CTA：${cta || '留言分享你的想法'}
${research?.insight ? `來賓定位：${research.insight}` : ''}
...
- 直接輸出純文字，不要 markdown 格式`;
    ...
    const { text: script, inputTokens, outputTokens } = await aiChat(prompt, { maxTokens: 2000 });

    addUsage('script', inputTokens, outputTokens);
