...
export default async function handler(req, res) {
  ...
  try {
    const { topic, job, cta, guest, interviewType, angles, research, user } = req.body;

    const anglesText = (angles || [])
      .map((a, i) => `Q${i + 1}. ${a.title}（${a.why}）追問方向：${a.howToShoot || '待定'}`)
      .join('\n');

    const prompt = `你是摩托車裝備 YouTube 頻道「摩托麻吉」的訪談腳本企劃。
請為以下訪談影片寫一份腳本大綱：
...
- 直接輸出純文字，不要 markdown 格式`;
    ...
    console.error('Interview script error:', err.message);
