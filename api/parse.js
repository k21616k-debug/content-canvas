...

{
  ...
  "sharedContext": "這批影片共用的背景知識或拍攝素材。沒有就寫空字串。",
  "gapSuggestions": [
    {
      "tempId": "gap1",
      "topic": "具體影片主題（可以直接使用的標題，不能太模糊）",
      "format": "long | short",
      "suggestedStage": "A | B | C | D",
      "suggestedJob": "吸引 | 培育 | 轉換",
      "gapReason": "一句話說明這支影片補了什麼缺口（例：你目前缺少 C 信任階段，觀眾看了評測還不夠相信你）"
    }
  ],
  "clipOpportunities": [
    {
  ...
  "missingInfo": "如果使用者的描述有重要資訊缺口，在這裡用一句話說明缺什麼。沒有缺口就寫空字串。"
}

## 購買旅程缺口分析

分析使用者描述的影片後，判斷四個階段的覆蓋狀況：
- A 認知：讓陌生人知道你和產品的存在（科普、介紹、懶人包）
- B 評估：讓有興趣的人比較和選擇（評測、比較、開箱）
- C 信任：讓考慮中的人相信你（使用心得、真實測試、時間驗證）
- D 安心：讓準備買的人放心下手（購買資訊、哪裡買、保固、常見問題）

gapSuggestions 規則：
- 只補使用者**完全沒有**的階段，不要重複他已有的內容
- 最多 2 個建議，優先補對轉換影響最大的缺口
- 每個建議要具體到可以直接開始規劃（不能寫「一支 C 信任的影片」這種空話）
- 如果使用者的影片已經覆蓋 3 個以上階段，gapSuggestions 給空陣列
- 建議的主題要和使用者描述的產品/主題相關，不能離題
